...
func (c *SearchUser) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "user/searchUser/searchUser.html"
	...
	if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
		...
		c.Data["SeacrhUsers"] = beego.AppConfig.String("ENGLISH_SEARCH_USERS")
		...
		c.Data["UserName"] = beego.AppConfig.String("ENGLISH_USER_FIRST_NAME")
		...
		c.Data["ListOfUsers"] = beego.AppConfig.String("ENGLISH_LIST_OF_USERS")
...
func (c *SearchUser) Post() {
	...
	columns := [8]string{"u.uuid", "u.first_name", "u.mobile", "u.email", "r.name", "u.language", "u.created_date", "u.status"}

	var sqlQuery bytes.Buffer
	var sqlArgs []interface{}
	sqlQuery.WriteString("select u.uuid,u.first_name,u.mobile,u.email,r.name,u.language,u.created_date,u.status from Users as u inner join Roles as r where (u.role_id = r.id)")
	if user_firstname != "" {
		sqlQuery.WriteString(" AND u.first_name like ?")
		sqlArgs = append(sqlArgs, user_firstname+"%")
	}
	if user_email != "" {
		sqlQuery.WriteString(" AND u.email like ?")
		sqlArgs = append(sqlArgs, user_email+"%")
	}
	if user_mobile != "" {
		sqlQuery.WriteString(" AND u.mobile  like ?")
	...
	if user_status != "" {
		sqlQuery.WriteString(" AND u.status=?")
	...
	if searchValue != "" {
		sqlQuery.WriteString(" AND (u.first_name like ? OR u.email like ? OR u.mobile like ? OR u.status like ? OR u.last_name like ? OR u.middle_name like ? OR u.language like ? OR r.name like ?)")
	...
	for i := range rowData {
		...
		if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
			if rowData[i][7] == "ACTIVE" {
				statusbadge = "<span class='badge badge-pill badge-success'>Active</span>"
			} else if rowData[i][7] == "INACTIVE" {
				statusbadge = "<span class='badge badge-pill badge-danger'>InActive</span>"
			} else if rowData[i][7] == "SUSPEND" {
				statusbadge = "<span class='badge badge-pill badge-warning'>Suspend</span>"
			} else {
				statusbadge = "--"
			}
			viewLink := "<a href='/User/ViewUser/" + rowData[i][0] + "'><h6 class='text-red' data-toggle='tooltip' data-placement='top' data-original-title='View'>" + rowData[i][0] + "</h6></a>"
			r = append(r, viewLink, rowData[i][1], rowData[i][2], rowData[i][3], rowData[i][4], rowData[i][5], rowData[i][6], statusbadge)
		} else if sess.Get("role") == "ADMIN" && sess.Get("language") == "French" {
			if rowData[i][7] == "ACTIVE" {
				...
				statusbadge = "<span class='badge badge-pill badge-warning'>Envoyer</span>"
			} else {
				statusbadge = "--"
			}
			viewLink := "<a href='/User/ViewUser/" + rowData[i][0] + "'><h6 class='text-red' data-toggle='tooltip' data-placement='top' data-original-title='Consulter'>" + rowData[i][0] + "</h6></a>"
			r = append(r, viewLink, rowData[i][1], rowData[i][2], rowData[i][3], rowData[i][4], rowData[i][5], rowData[i][6], statusbadge)
		}

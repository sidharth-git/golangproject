...
func (c *UpdatePackage) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "packages/updatePackage/updatePackage.html"
...
func (c *UpdatePackage) Post() {
	...
	defer func() {
		...
		if err != nil {
			...
			if sess.Get("language") == "English" {
				c.Data["DisplayMessage"] = beego.AppConfig.String("ENGLISH_PACKAGE_UPDATED_SUCCESSFULLY")
				c.TplName = "packages/updatePackage/updatePackage.html"
				log.Println(beego.AppConfig.String("loglevel"), "Info", "Update Admin User  Page Success")
	...
	user_last_name := c.Input().Get("input_user_last_name1")
	...
	if !utils.IsLetter(user_first_name) {
	...
	if AdminId == beego.AppConfig.String("CNPS_DEFAULT_USER_ID") || AdminId == beego.AppConfig.String("CNPS_DEFAULT_MERCHANT_ID") {
		...
		if user_status == "ACTIVE" {
			result, err := db.Db.Exec(`UPDATE Txn_Package SET name=?, volume =?,txn_fees=?,status=? WHERE id= ?`,
				user_first_name, user_middele_name, user_last_name,
				user_status, AdminId)
			if err != nil {
				err = errors.New("Package updation failed")
				return
			}
			...
			//result, err := db.Db.Exec(`UPDATE Txn_Package SET name=?, volume =?,txn_fees=?,status=?,updated_at=now() WHERE id= ?`,
			//	user_first_name, user_middele_name, user_last_name,
			//user_status)
			result, err := db.Db.Exec(`UPDATE Txn_Package SET name=?, volume =?,txn_fees=?,status=? WHERE id= ?`,
				user_first_name, user_middele_name, user_last_name,
...
func Getuserdata(c *UpdatePackage, AdminId string) {
	row, err := db.Db.Query(`select id,name,volume,txn_fees,status from Txn_Package  where id= ?`, AdminId)
	if err != nil {
		log.Println(beego.AppConfig.String("loglevel"), "Error", err)
		err = errors.New("Unable to get Package data")
		return

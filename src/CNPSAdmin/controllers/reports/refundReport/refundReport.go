...
func (c *RefundReport) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "reports/refundReport/refundReport.html"
	...
	auth, err := utils.IsAuthorized(sess.Get("menujson").(string), "RefundReport")
	...
	if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
		...
		c.Data["RefundReport"] = beego.AppConfig.String("ENGLISH_REFUND_REPORT")
		...
		c.Data["RefundReport"] = beego.AppConfig.String("FRENCH_REFUND_REPORT")
...
func (c *RefundReport) Post() {
	...
	columns := [7]string{"merchant_id", "refund_id", "bank_txn_id", "amount", "created_at", "updated_at", "status"}

	var sqlQuery bytes.Buffer
	var sqlArgs []interface{}
	sqlQuery.WriteString("select merchant_id, refund_id, bank_txn_id, amount, created_at, updated_at, status FROM Refund WHERE merchant_id IS NOT NULL")
	if merchantId != "" {
	...
	for i := range rowData {
		...
		if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
			...
			r = append(r, rowData[i][0], rowData[i][1], rowData[i][2], rowData[i][3], rowData[i][4], rowData[i][5], statusbadge)

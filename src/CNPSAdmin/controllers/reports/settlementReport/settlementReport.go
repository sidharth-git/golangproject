...
func (c *SettlementReport) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "reports/settlementReport/settlementReport.html"
	...
	auth, err := utils.IsAuthorized(sess.Get("menujson").(string), "SettlementReport")
	...
	if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
		...
		c.Data["SettlementReport"] = beego.AppConfig.String("ENGLISH_SETTLEMENT_REPORT")
		...
		c.Data["SettlementReport"] = beego.AppConfig.String("FRENCH_SETTLEMENT_REPORT")
...
func (c *SettlementReport) Post() {
	...
	amount := c.Input().Get("input_amount_in_pgs_base_currency")
	serviceCharge := c.Input().Get("input_servicecharge_in_pgs_base_currency")
	...
	columns := [12]string{"merchant_id", "settlement_id", "amount_in_base_currency", "service_charge_in_base_currency", "settlement_currency", "amount_in_settlement_currency", "service_charge_in_settlement_currency", "conversion_rate", "created_at", "bank_txn_id", "updated_at", "status"}

	var sqlQuery bytes.Buffer
	var sqlArgs []interface{}
	sqlQuery.WriteString("select merchant_id, settlement_id, amount_in_base_currency, service_charge_in_base_currency,settlement_currency,amount_in_settlement_currency,service_charge_in_settlement_currency,conversion_rate, created_at, bank_txn_id, updated_at,status FROM Settlement WHERE merchant_id IS NOT NULL")
	if merchantId != "" {
	...
	if serviceCharge != "" {
		...
		sqlArgs = append(sqlArgs, serviceCharge)
	}
	...
	if searchValue != "" {
		sqlQuery.WriteString(" AND (merchant_id like ? OR settlement_id like ? OR amount_in_base_currency like ? OR service_charge_in_base_currency like ? OR settlement_currency like ? OR bank_txn_id like ? OR status like ?)")
	...
	for i := range rowData {
		...
		if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
			...
			r = append(r, rowData[i][0], rowData[i][1], rowData[i][2], rowData[i][3], rowData[i][4], rowData[i][5], rowData[i][6], rowData[i][7], rowData[i][8], rowData[i][9], rowData[i][10], statusbadge)
			...
			r = append(r, rowData[i][0], rowData[i][1], rowData[i][2], rowData[i][3], rowData[i][4], rowData[i][5], rowData[i][7], rowData[i][8], rowData[i][9], rowData[i][10], rowData[i][6], statusbadge)
			result = append(result, r)

...
func (c *UpdateRole) Get() {
	...
	log.Println(beego.AppConfig.String("loglevel"), "Info", "Update Role Page Start")
	pip := c.Ctx.Input.IP()
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "role/updateRole/updateRole.html"
	...
	auth, err := utils.IsAuthorized(sess.Get("menujson").(string), "Updaterole")
...
func (c *UpdateRole) Post() {
	...
	log.Println(beego.AppConfig.String("loglevel"), "Debug", "role name - ", department_name)
	...
	// }
	// rowRole, err := db.Db.Query(`select name from Roles where name= ?`+orCond, roleName)

	rowRole, err := db.Db.Query(`select count(name) from Roles where name= ?`, roleName)
	...
	defer sql.Close(rowRole)
	_, RoleData, err := sql.Scan(rowRole)
	if err != nil {
		log.Println(beego.AppConfig.String("loglevel"), "Error", err)
		err = errors.New("Unable to get role data")
		return
	}
	log.Println(beego.AppConfig.String("loglevel"), "Debug", "Query Data - ", RoleData, "\nData len - ", len(RoleData))
	...
	if erole != "0" {
		if sess.Get("language").(string) == "English" {
			...
			c.Data["DisplayMessage"] = beego.AppConfig.String("ENGLISH_RoleAlreadyExist")
		} else {
			err = errors.New(beego.AppConfig.String("FRENCH_RoleAlreadyExist"))
			c.Data["DisplayMessage"] = beego.AppConfig.String("FRENCH_RoleAlreadyExist")
		}
	} else {

		result, err := db.Db.Exec(`UPDATE Roles SET name =?,last_update=now(),updated_by=? WHERE uuid= ?`,
			department_name, user_email, AdminId)
		if err != nil {

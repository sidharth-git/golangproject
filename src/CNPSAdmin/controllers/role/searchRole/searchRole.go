...
func (c *SearchRole) Get() {
	log.Println(beego.AppConfig.String("loglevel"), "Info", "Search Role Start")
	pip := c.Ctx.Input.IP()
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "role/searchRole/searchRole.html"
			log.Println(beego.AppConfig.String("loglevel"), "Info", "Search Role Page Fail")
		} else {
			c.Data["DisplayMessage"] = ""
			c.TplName = "role/searchRole/searchRole.html"
			log.Println(beego.AppConfig.String("loglevel"), "Info", "SSearch Role Page Success")
		}
		return
	}()
	...
	auth, err := utils.IsAuthorized(sess.Get("menujson").(string), "Viewrole")
	...
	if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
		...
		c.Data["ListOfRoles"] = beego.AppConfig.String("ENGLISH_LIST_OF_ROLES")
		...
		c.Data["ListOfRoles"] = beego.AppConfig.String("FRENCH_LIST_OF_ROLES")
...
func (c *SearchRole) Post() {
	log.Println(beego.AppConfig.String("loglevel"), "Info", "Search Role Page Start")
	pip := c.Ctx.Input.IP()
	log.Println(beego.AppConfig.String("loglevel"), "Debug", "Client IP - ", pip)
	var err error
	sessErr := false
	...
	for i := range rowData {
		var r []string
		if sess.Get("role") == "ADMIN" && sess.Get("language") == "English" {
			viewLink := "<a href='" + beego.URLFor("ViewRole.Get", ":AdminID", rowData[i][0]) + "'><h6 class='text-red' data-toggle='tooltip' data-placement='top' data-original-title='View'>" + rowData[i][0] + "</h6></a>"
			r = append(r, viewLink, rowData[i][1], rowData[i][2], rowData[i][3])
		} else if sess.Get("role") == "ADMIN" && sess.Get("language") == "French" {
			viewLink := "<a href='" + beego.URLFor("ViewRole.Get", ":AdminID", rowData[i][0]) + "'><h6 class='text-red' data-toggle='tooltip' data-placement='top' data-original-title='Consulter'>" + rowData[i][0] + "</h6></a>"
			r = append(r, viewLink, rowData[i][1], rowData[i][2], rowData[i][3])
		}
